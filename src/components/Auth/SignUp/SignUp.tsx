...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
        const response = await fetch('http://ludoviclebris-server.eddi.cloud/api/api/users/signup', {
            method: 'POST',
            headers: {
        ...
        <label className='signUp-back-btn'><IoIosArrowBack/></label> 
          ...
          <div className="signUp-content">
            <div className='signUp-pseudo'>
              <input type="text" placeholder='Pseudo' onChange={(e) => setPseudonym(e.target.value)} value={pseudonym}/>
              </div>
              ...
              <input type="email" placeholder='Adresse e-mail' onChange={(e) => setEmail(e.target.value)} value={email}/>
